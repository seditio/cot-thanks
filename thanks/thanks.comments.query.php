...
if (Cot::$cfg['plugin']['thanks']['comments_on'] && Cot::$cfg['plugin']['thanks']['comments_order']) {
	...
	if ($usr['id'] > 0 && cot_auth('plug', 'thanks', 'W')) {
		$comments_join_columns .= ", (SELECT COUNT(*) FROM `$db_thanks` WHERE `th_fromuser` = {$usr['id']} AND `th_touser` = c.com_authorid AND DATE(`th_date`) = DATE(NOW())) AS thanks_touser_today, (SELECT COUNT(*) FROM `$db_thanks` WHERE `th_fromuser` = {$usr['id']} AND `th_ext` = 'comments' AND `th_item` = c.com_id) AS thanks_toitem";
	}
